...
export const createEmployeeSchema = z.object({
  body: z.object({
    ...
    position_id: z.string().or(z.number()).transform((val) => Number(val)).refine((n) => !isNaN(n) && n > 0, "Position is required"),
    department_id: z.string().or(z.number()).transform((val) => Number(val)).refine((n) => !isNaN(n) && n > 0, "Department is required"),
    role_id: z.string().or(z.number()).transform((val) => Number(val)).refine((n) => !isNaN(n) && n > 0, "Role is required"),
