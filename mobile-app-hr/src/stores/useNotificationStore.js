  ...
  initSocket: async () => {
    try {
      ...
      newSocket.on('leave:updated', (data) => {
        console.log('[NotificationStore] Real-time leave:updated received:', data);
        try {
          const useLeaveStore = require('./useLeaveStore').default;
          if (useLeaveStore) {
            useLeaveStore.getState().fetchLeaveHistory().catch(() => {});
            useLeaveStore.getState().fetchLeaveSummary().catch(() => {});
            useLeaveStore.getState().fetchPendingLeaves().catch(() => {});
          }
        } catch (e) {}
      });

      newSocket.on('overtime:updated', (data) => {
        console.log('[NotificationStore] Real-time overtime:updated received:', data);
        try {
          const useOvertimeStore = require('./useOvertimeStore').default;
          if (useOvertimeStore) {
            useOvertimeStore.getState().fetchOvertimeHistory().catch(() => {});
            useOvertimeStore.getState().fetchPendingOvertimes().catch(() => {});
          }
        } catch (e) {}
      });
